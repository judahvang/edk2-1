...
fips_test_status fips_selftest_mbx_rsa2k_private_mb8(void)
{
    ...
    sts = mbx_rsa_private_mb8(
        pa_ciphertext, pa_plaintext, pa_d, pa_moduli, MBX_RSA2K_DATA_BIT_LEN, method, NULL);
    ...
    for (int i = 0; (i < MBX_LANES) && (MBX_ALGO_SELFTEST_OK == test_result); ++i) {
        output_status = mbx_is_mem_eq(
            pa_plaintext[i], MBX_RSA2K_DATA_BYTE_LEN, plaintext, MBX_RSA2K_DATA_BYTE_LEN);
...
fips_test_status fips_selftest_mbx_rsa2k_private_ssl_mb8(void)
{
    ...
    sts = mbx_rsa_private_ssl_mb8(
        pa_ciphertext, pa_plaintext, pa_d, pa_moduli, MBX_RSA2K_DATA_BIT_LEN);
    test_result = mbx_selftest_check_if_success(sts, MBX_ALGO_SELFTEST_BAD_ARGS_ERR);
