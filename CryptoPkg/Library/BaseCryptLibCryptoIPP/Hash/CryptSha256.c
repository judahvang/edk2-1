  ...
  )
{
  ...
  //
  return (UINTN)(sizeof (IppsHashState_rmf));
}
  ...
  )
{
  ...
  Status = ippsHashInit_rmf ((IppsHashState_rmf *)Sha256Context, ippsHashMethod_SHA256 ());

  if (Status != ippStsNoErr) {
    DEBUG ((DEBUG_INFO, "%a: %d Status: 0x%x\n",
  ...
  )
{
  ...
  Status = ippsHashDuplicate_rmf (
    (IppsHashState_rmf *)Sha256Context,
    (IppsHashState_rmf *)NewSha256Context
    );
  ...
  )
{
  ...
  Status = ippsHashUpdate_rmf ((const Ipp8u *)Data, (int)DataSize, (IppsHashState_rmf *)Sha256Context);
  ...
  )
{
  ...
  if (!Sha256Update (&Context, Data, DataSize)) {
    return FALSE;
  }

  if (!Sha256Final (&Context, HashValue)) {
    return FALSE;
  }
