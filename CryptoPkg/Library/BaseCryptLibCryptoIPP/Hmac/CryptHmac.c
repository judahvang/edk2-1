  ...
  )
{
  ...
  if (ippsHMACGetSize_rmf (&pSize) != ippStsNoErr) {
    HmacMdCtx = AllocateZeroPool (pSize);
    if (HmacMdCtx == NULL) {
      return NULL;
    }
  ...
  )
{
  ...
  ZeroMem (HmacMdContext, sizeof (ippsHMACGetSize_rmf (HmacMdContext)));
  ippsHMACInit_rmf (Key, KeySize, HmacMdContext);
